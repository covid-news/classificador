...
trainingNews = pd.read_excel('Training Set_v4 SW Stemming.xlsx', header = None, names = ['A', 'B'])
testNews = pd.read_excel( 'Test Set_v2 SW Stemming.xlsx', header = None, names = ['A', 'B'] )
#file = open( 'Classificador.pickle','wb' )
"""book = load_workbook( 'Test Set.xlsx' )
sheet = book['Plan1']"""
clf = NaiveBayesClassifier(trainingNews.values, format = ".xlsx")
with open( r"Classificador.pickle",'wb' ) as file:
    pickle.dump( clf, file, protocol = pickle.HIGHEST_PROTOCOL, fix_imports = False )
...
workbook = xlsxwriter.Workbook( 'Test Results_v6 SW Stemming.xlsx' )
